...
def add_book():
  conn = mysql.connector.connect(host='localhost', database='library', user='root', password = "Anshul@821086")
  cursor = conn.cursor()
  ...
  sql = 'insert into book(book_id,title,author,price,pages,publisher,edition,status) values ( "' + \
       book_id + '","'+title + '","' + author+'",'+price+','+pages+',"'+publisher+'","'+edition+'","available");'
...
def add_member():
  ...
  sql = 'insert into member(mem_id,name,class,address,phone,email) values ( "' + \
      mem_id + '","'+name + '","' + clas+'","'+address+'","'+phone + '","'+email+'");'
  cursor.execute(sql)
  conn.close()
...
def modify_member():
    ...
    mem_id =input('Enter member ID : ')
    value = input('Enter new value : ')
    sql = 'update member set '+ field +' = "'+value+'" where mem_id = '+mem_id+';'
    cursor.execute(sql)
...
def mem_issue_status(mem_id):
    conn = mysql.connector.connect(host='localhost', database='library', user='root', password = "Anshul@821086")
    cursor = conn.cursor()
    sql ='select * from transaction where m_id ='+str(mem_id) +' and dor is NULL;'
    cursor.execute(sql)
...
def book_status(book_id):
    conn = mysql.connector.connect(host='localhost', database='library', user='root', password = "Anshul@821086")
    cursor = conn.cursor()
    sql = 'select * from book where book_id ='+str(book_id)+ ';'
    cursor.execute(sql)
...
def issue_book():
    ...
    if len(result1) == 0:
      if result == 'available':
          sql = 'insert into transaction(tid,b_id, m_id, doi) values('+str(trans_id)+','+str(book_id)+','+str(mem_id)+',"'+str(today)+'");'
          sql_book = 'update book set status="issue" where book_id ='+str(book_id) + ';'
          cursor.execute(sql)
      ...
      if len(result1)<1:
        ...
        sql_book = 'update book set status="issue" where mem_id ='+book_id + ';'
        #print(len(result))
        cursor.execute(sql)
        cursor.execute(sql_book)
        print('\n\n\n Book issued successfully')
...
def change_book_status(status,book_id):
    conn = mysql.connector.connect(
        host='localhost', database='library', user='root', password = "Anshul@821086")
    cursor = conn.cursor()
    sql = 'update book set status = "'+status +'" where book_id ='+book_id + ' and status ="available"'
    cursor.execute(sql)
